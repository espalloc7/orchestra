...
const codexNote = codex
  ? [
      ...
      '  - This one subagent covers every Codex job: implementation, code review, and',
      '    adversarial review. Set the job in the prompt. There is nothing the /codex:',
      '    review commands do that a codex-rescue prompt cannot.',
      ...
      '',
      'Run the Codex work yourself through the subagent. Never ask the user to type a',
      '/codex: command as a stand-in for that — waiting on a human to drive Codex is the',
      'exact stall this route exists to remove. A review the lead could have run through',
      'codex-rescue is not a task to hand back to the user.',
      '',
      'The /codex: slash commands are user-typed and disable-model-invocation, so the lead',
      'cannot run them. They are not a delegation path. The only reason to mention one is',
      'job control over a job the USER started, which the lead has no other handle on:',
      '',
      '  /codex:status | :result | :cancel   inspect or stop a user-started job',
      '',
